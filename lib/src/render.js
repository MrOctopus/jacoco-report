...
function getOverallTable(project, minCoverage, emoji) {
    ...
    if ((project.changed.missed + project.changed.covered) !== 0) {
        const filesChangedStatus = getStatus(project['coverage-changed-files'], minCoverage.changed, emoji);
        changedCoverageRow =
            '\n' +
                `|Files changed|${formatCoverage(project['coverage-changed-files'])}|${filesChangedStatus}|` +
                '\n<br>';
    }
