...
function action() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            let base = core.getInput('base-sha');
            let head = core.getInput('head-sha');
            let prNumber;
            if (base || head) {
                if (!base && !head) {
                    core.setFailed(`When overriding either base-sha or head-sha, both values must be set.`);
                    return;
                }
            }
            else {
                switch (event) {
                    ...
                    case 'push':
                        ...
                        break;
                    default:
                        core.setFailed(`Only pull requests and pushes are supported with automatic detection, ${github.context.eventName} not supported.`);
                        return;
                }
            }
